    ...
    unsigned long iter)
{
  ...
  default_values[4] = default_values_t(pedestal, -500, 500);
...
/*
  ...
  */
  fits.SetParLimits(0, -500, 500);
  //fits.SetParNames("Amplitude", "Sample Start", "Power", "Peak Time 1", "Pedestal", "Amplitude ratio", "Peak Time 2");
  fits.FixParameter(1, 100);
  fits.FixParameter(2, 2);
  fits.FixParameter(3, 120);
  //fits.FixParameter(4, 0);
  fits.FixParameter(5, 0);
  fits.FixParameter(6, 120);
  if (verbosity <= 1)
    gpulse.Fit(&fits, "RN0W", "goff", 0., (double) NSAMPLES);
  else
  ...
  if (verbosity)
  {
    ...
    string histo_name = "Graph" + to_string(iter);
    canvas -> Write(histo_name.c_str());
    tfile -> Write();
...
*/
  ...
  if (verbosity)
  {
    cout << "Second verbosity" << endl;
    cout << "PROTOTYPE4_FEM::SampleFit_PowerLawDoubleExp - "
         << "peak_sample = " << peak_sample << ", "
         //m<< "max_peakpos = " << max_peakpos << ", "
